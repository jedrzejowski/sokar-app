...
void Monochrome::Scene::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
    ...
    if (event->buttons() & Qt::LeftButton) {
        ...
        switch (getDicomView()->getToolBar()->getState()) {
            ...
            case SokarUi::DicomToolBar::Windowing: {
                event->accept();
...
void Monochrome::Scene::toolBarActionSlot(SokarUi::DicomToolBar::Action action, bool state) {

    SokarScene::DicomScene::toolBarActionSlot(action, state);

    if (action == SokarUi::DicomToolBar::Segmentation) {
