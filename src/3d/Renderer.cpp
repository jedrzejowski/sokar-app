...
//}
//
//Renderer::Renderer(VulkanWidget *w, int initialCount)
//		: m_window(w),
...
//	);
//
//	qDebug() << m_blockMesh.data()->sizeInBytes();
//
...
//		m_itemMaterial.vs.load(inst, dev, QStringLiteral(":/vk-shader/color_phong.vert.spv"));
//	if (!m_itemMaterial.fs.isValid())
//		m_itemMaterial.fs.load(inst, dev, QStringLiteral(":/vk-shader/color_phong.frag.spv"));
//
//	if (!m_floorMaterial.vs.isValid())
//		m_floorMaterial.vs.load(inst, dev, QStringLiteral(":/vk-shader/color_vert.spv"));
//	if (!m_floorMaterial.fs.isValid())
//		m_floorMaterial.fs.load(inst, dev, QStringLiteral(":/vk-shader/color_frag.spv"));
//
...
//	err = m_devFuncs->vkCreateDescriptorPool(dev, &descPoolInfo, nullptr, &m_itemMaterial.descPool);
//	if (err != VK_SUCCESS)
...
//
//	err = m_devFuncs->vkCreateGraphicsPipelines(dev, m_pipelineCache, 1, &pipelineInfo, nullptr,
//												&m_itemMaterial.pipeline);
//	if (err != VK_SUCCESS)
...
//
//	VkResult err = m_devFuncs->vkCreatePipelineLayout(dev, &pipelineLayoutInfo, nullptr,
//													  &m_floorMaterial.pipelineLayout);
//	if (err != VK_SUCCESS)
//		qFatal("Failed to create pipeline layout: %d", err);
//
//	VkGraphicsPipelineCreateInfo pipelineInfo;
...
//	const int blockMeshByteCount = m_blockMesh.data()->sizeInBytes();
...
//	const int logoMeshByteCount = m_logoMesh.data()->sizeInBytes();
...
//	}
//
//	m_devFuncs->vkCmdDraw(cb, (m_useLogo ? m_logoMesh.data() : m_blockMesh.data())->sizeInBytes(), m_instCount, 0, 0);
//}
//
...
//	m_devFuncs->vkCmdPushConstants(cb, m_floorMaterial.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, 64,
//								   mvp.constData());
